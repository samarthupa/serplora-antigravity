...
export default function SettingsOverlay({ currentSettings, onSave, onClose, isMobile, onReset, onShare, isSharing }: any) {
  ...
  const ToggleSwitch = ({ checked, onChange }: { checked: boolean, onChange: () => void }) => (
    <div 
      onClick={onChange}
      className={`w-10 h-[22px] flex items-center rounded-full p-1 cursor-pointer transition-colors duration-200 ease-in-out ${checked ? 'bg-[#007acc]' : 'bg-gray-300 dark:bg-[#555]'}`}
    >
      ...
      <div 
        ...
        className={`${
          isMobile 
            ...
            : 'absolute bottom-14 left-14 rounded-md w-64'
        } bg-white dark:bg-[#252526] shadow-2xl z-[99999] p-5 text-[14px] text-gray-800 dark:text-[#cccccc] animate-in ${isMobile ? 'slide-in-from-bottom-8' : 'fade-in zoom-in'} duration-200`}
      >
        {/* Mobile drag handle indicator */}
        {isMobile && <div className="w-12 h-1.5 bg-gray-300 dark:bg-[#454545] rounded-full mx-auto mb-5" />}

        {/* Mobile-Only Workspace Actions */}
        {isMobile && (
          <div className="mb-5 pb-5 border-b border-gray-200 dark:border-[#3c3c3c] flex justify-between gap-3">
            <button 
              onClick={() => { onClose(); onReset(); }} 
              className="flex-1 flex items-center justify-center gap-2 py-2 px-2 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-lg transition-colors text-[13px] font-semibold"
            >
        ...
        <h4 className="font-bold mb-4 text-[14px] text-gray-900 dark:text-white border-b border-gray-200 dark:border-[#3c3c3c] pb-2">
          Editor Settings
        </h4>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <ToggleSwitch checked={draft.wordWrap} onChange={() => setDraft({ ...draft, wordWrap: !draft.wordWrap })} />
          </div>

          <div className="flex items-center justify-between">
            <span className="font-medium">Autocomplete</span>
            <ToggleSwitch checked={draft.autoComplete} onChange={() => setDraft({ ...draft, autoComplete: !draft.autoComplete })} />
          </div>

          <div className="flex items-center justify-between">
            <span className="font-medium">Line Numbers</span>
            <ToggleSwitch checked={draft.showGutter} onChange={() => setDraft({ ...draft, showGutter: !draft.showGutter })} />
          </div>
        </div>

        <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-200 dark:border-[#3c3c3c]">
          <button onClick={handleReset} className="text-[#007acc] hover:underline text-[13px] font-medium">Reset Default</button>
          <button onClick={handleSave} className="bg-[#007acc] hover:bg-[#005f9e] text-white px-5 py-2 rounded-lg transition-colors font-semibold shadow-sm">Save Preferences</button>
        </div>
      </div>
    </>
  );
